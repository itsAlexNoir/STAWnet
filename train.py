...
parser.add_argument('--gat_bool', default=True,
                    help='whether to add graph attention layer')
parser.add_argument('--aptonly', default=False,
                    help='whether only use node embedding to do attention')
parser.add_argument('--noapt', default=False,
                    help='whether not use node embedding to do attention')
parser.add_argument('--addaptadj', default=True,
                    help='whether add adaptive adj')
parser.add_argument('--randomadj', default=True,
                    help='whether random initialize adaptive adj')
parser.add_argument('--seq_length', type=int, default=12, help='')
parser.add_argument('--nhid', type=int, default=32, help='')
parser.add_argument('--emb_length', type=int, default=16,
                    help='node embedding length')
parser.add_argument('--in_dim', type=int, default=3, help='inputs dimension')
parser.add_argument('--num_nodes', type=int, default=207,
                    help='number of nodes, METR:207, PEMS:325')
...
parser.add_argument('--save', type=str,
                    default='./garage/PEMS_addrelu', help='save path')
...
def main():
    ...
    for i in range(1, args.epochs+1):
        ...
        torch.save(engine.model.state_dict(), args.save+"_epoch_" +
                   str(i)+"_"+str(round(mvalid_loss, 2))+".pth")
        writer.add_scalars(
            'scalar/test', {'train_loss': mtrain_loss, 'val_loss': mvalid_loss}, i)
    writer.close()
    print(
    ...
    engine.model.load_state_dict(torch.load(
        args.save+"_epoch_"+str(bestid+1)+"_"+str(round(his_loss[bestid], 2))+".pth"))
    ...
    torch.save(engine.model.state_dict(), args.save+"_exp" +
