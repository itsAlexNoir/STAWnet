...
class graphattention(nn.Module):
    ...
    def forward(self, x, embedding):
        ...
        # embedding = [batch_size, time_step, 10, num_nodes]
        embedding = embedding.repeat((x.shape[0], x.shape[-1], 1, 1))
        # embedding = [batch_size, 16, num_nodes, time_step]
        ...
        if self.aptonly:
            ...
            # value = self.vm(x)
            # attention=[batch_size, time_step, num_nodes, num_nodes]
            ...
            x_embedding = x
            # query=[batch_size, time_step, num_nodes, d]
            query = self.qm(x_embedding).permute(0, 3, 2, 1)
            # key=[batch_size, time_step, num_nodes, d]
            key = self.km(x_embedding).permute(0, 3, 2, 1)
            # attention=[batch_size, time_step, num_nodes, num_nodes]
            attention = torch.matmul(query, key.permute(0, 1, 3, 2))
            # attention = F.relu(attention)
            ...
            # x_embedding=[batch_size, D+10, num_nodes, time_step]
...
class stawnet(nn.Module):
    def __init__(self, device, num_nodes, dropout=0.3, supports=None, gat_bool=True, addaptadj=True, aptonly=False, noapt=False, aptinit=None, in_dim=2, out_dim=12, residual_channels=32, dilation_channels=32, skip_channels=256, end_channels=512, kernel_size=2, blocks=4, layers=2, emb_length=16):
        ...
        if gat_bool and addaptadj:
            self.embedding = nn.Parameter(torch.randn(self.emb_length, num_nodes).to(
        ...
        for b in range(blocks):
            ...
            for i in range(layers):
                ...
                additional_scope *= 2
                if self.gat_bool:
                    self.gat.append(graphattention(dilation_channels, residual_channels,
                                    dropout, emb_length=emb_length, aptonly=aptonly, noapt=noapt))
